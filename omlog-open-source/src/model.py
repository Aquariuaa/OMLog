...
class OMLog(BaseModel):
    def __init__(self, options):
        super(OMLog, self).__init__(options)
    ...
    def forward(self, input_dict):
        ...
        max_num_candidates_label = max([next_log['eventid'] for next_log in input_dict['next']])
        if self.num_candidates <= max_num_candidates_label:
            self.num_candidates = max_num_candidates_label + 1
